...
def moon_pointing(): 
    ...
    num = 0
    #now we want to track the Moon
    while running:
            num += 1 
            jd = ugradio.timing.julian_date()   #get the new Julian date after pointing at the Moon initially
            julians.update({num:jd})
            #first we want to find the Moon
            #get the altitude and azimuth of the sun at the given jd. Got precessed to correct for precession of equinox 
            ra, dec = ugradio.coord.moonpos(jd = jd)
